...
class UnrealConnection:
    ...
    def send_command(self, command: str, params: Dict[str, Any] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # v0.8.0 strict wire shape — the bridge already normalizes every
            # response into `{success: bool, error?, ...payload}`. Just log
            # errors here for debugging and pass the dict through unchanged.
            if response.get("success") is False:
                logger.error(
                    f"Unreal error: {response.get('error', 'Unknown Unreal error')}"
                )

            # Always close the connection after the command is complete —
            # Unreal closes its side anyway after responding.
            try:
                self.socket.close()
            except:
                pass
...
@mcp.prompt()
def info():
    """High-level pointer for the UnrealMCP server.
    ...
    Keeping a parallel prose summary in this prompt drifts out of date
    almost immediately as tools are added, so we deliberately don't try.
    """
    return """
# Unreal MCP

Programmatic equivalent of the Unreal Editor UI — inspect, navigate,
query, create, modify, organize, run, observe.

Categories: editor (actors, viewport, console, PIE, screenshots),
assets (registry, mutations, migration, import), materials, outliner,
levels, blueprints + nodes, UMG widgets, project settings.

Use `tools/list` for the authoritative catalog (every tool's docstring
is the surface). Wire format is strict: success → `{"success": true, ...}`,
