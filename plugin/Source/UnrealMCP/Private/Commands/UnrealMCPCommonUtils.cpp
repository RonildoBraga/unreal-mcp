...
{
    // v0.8.0 -- thin wrapper over the FPropertyWalker backend. Was a 350-line
    // FProperty type switch (v0.7.4 era), now superseded by Day 2a's lifted
    // PropertyWalker which already handles every leaf type plus the dotted-
    // path and component-fallback paths the blueprint commands need.
    //
    // Callers in UnrealMCPBlueprintCommands.cpp keep working unchanged --
    // the external contract (UObject*, single-segment FString name, JsonValue,
    // out-error) is identical. The 350-line FProperty type switch was a
    // straight duplicate of FPropertyWalker::SetValue.
    if (!Object)
    {
    ...
    if (!WalkPropertyPath(Object, PropertyName, Target, OutErrorMessage))
    {
        return false;
    }
    return SetPropertyAtTarget(Target, Value, OutErrorMessage);
}
