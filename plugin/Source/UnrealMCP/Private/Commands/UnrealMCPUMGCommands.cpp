// v0.8.x §6.2 completion -- UMG widget command handlers, lifted out of the
// v0.7-era FUnrealMCPUMGCommands class.
...
bool ReadVector2DField(const TSharedPtr<FJsonObject>& Obj, const FString& Field, FVector2D& Out)
{
    ...
    if (!Obj->TryGetArrayField(Field, Arr) || !Arr || Arr->Num() < 2) return false;
...
// ─── v1 handlers (path inferred as /Game/Widgets/<BlueprintName>) ──────────
...
TSharedPtr<FJsonObject> HandleCreateUMGWidgetBlueprint(const TSharedPtr<FJsonObject>& Params)
{
    ...
    UBlueprint* NewBlueprint = FKismetEditorUtilities::CreateBlueprint(
        UUserWidget::StaticClass(), Package, FName(*AssetName),
        BPTYPE_Normal, UBlueprint::StaticClass(),
        UBlueprintGeneratedClass::StaticClass(), FName("CreateUMGWidget"));
...
TSharedPtr<FJsonObject> HandleBindWidgetEvent(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    { Response->SetStringField(TEXT("error"), TEXT("Missing blueprint_name parameter")); return Response; }
    if (!Params->TryGetStringField(TEXT("widget_name"), WidgetName))
    { Response->SetStringField(TEXT("error"), TEXT("Missing widget_name parameter")); return Response; }
    if (!Params->TryGetStringField(TEXT("event_name"), EventName))
    { Response->SetStringField(TEXT("error"), TEXT("Missing event_name parameter")); return Response; }
    ...
    if (!EventGraph)
    { Response->SetStringField(TEXT("error"), TEXT("Failed to find or create event graph")); return Response; }

    UWidget* Widget = WidgetBlueprint->WidgetTree->FindWidget(*WidgetName);
    if (!Widget)
    { Response->SetStringField(TEXT("error"), FString::Printf(TEXT("Failed to find widget: %s"), *WidgetName)); return Response; }
    ...
    if (!EventNode)
    { Response->SetStringField(TEXT("error"), TEXT("Failed to create event node")); return Response; }
...
TSharedPtr<FJsonObject> HandleSetTextBlockBinding(const TSharedPtr<FJsonObject>& Params)
{
    ...
    { Response->SetStringField(TEXT("error"), TEXT("Missing binding_name parameter")); return Response; }

    const FString BlueprintPath = FString::Printf(TEXT("/Game/Widgets/%s.%s"), *BlueprintName, *BlueprintName);
    UWidgetBlueprint* WidgetBlueprint = Cast<UWidgetBlueprint>(UEditorAssetLibrary::LoadAsset(BlueprintPath));
    if (!WidgetBlueprint)
    { Response->SetStringField(TEXT("error"), FString::Printf(TEXT("Failed to load Widget Blueprint: %s"), *BlueprintPath)); return Response; }

    FBlueprintEditorUtils::AddMemberVariable(
        WidgetBlueprint, FName(*BindingName),
    ...
    if (!TextBlock)
    { Response->SetStringField(TEXT("error"), FString::Printf(TEXT("Failed to find TextBlock widget: %s"), *WidgetName)); return Response; }

    const FString FunctionName = FString::Printf(TEXT("Get%s"), *BindingName);
    UEdGraph* FuncGraph = FBlueprintEditorUtils::CreateNewGraph(
        WidgetBlueprint, FName(*FunctionName), UEdGraph::StaticClass(), UEdGraphSchema_K2::StaticClass());
...
// ─── v2 handlers (path-flexible, parent-aware, with property setters) ──────
...
TSharedPtr<FJsonObject> HandleAddWidgetToTree(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (!WBP)
        return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Could not load WBP at '%s'"), *Path));

    UClass* WClass = ResolveWidgetClass(WidgetClassName);
    if (!WClass)
        return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Unknown widget_class '%s'"), *WidgetClassName));
    ...
    if (Params->TryGetStringField(TEXT("parent_name"), ParentName) && !ParentName.IsEmpty())
    {
        ...
        if (!Parent)
            return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Parent '%s' not found or not a PanelWidget"), *ParentName));
    }
    else
    {
        ...
        if (!Parent)
        {
            ...
            {
                UCanvasPanel* NewRoot = WBP->WidgetTree->ConstructWidget<UCanvasPanel>(UCanvasPanel::StaticClass(), FName(*WidgetName));
            ...
            return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Root widget missing; create a CanvasPanel first"));
        }
    }
    ...
    // Register in GUID map BEFORE compile -- v0.7.10 fix.
    WBP->WidgetVariableNameToGuidMap.Add(NewWidget->GetFName(), FGuid::NewGuid());

    UPanelSlot* AddedSlot = Parent->AddChild(NewWidget);
    if (!AddedSlot)
        return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Could not add to parent '%s'"), *ParentName));
...
TSharedPtr<FJsonObject> HandleSetWidgetText(const TSharedPtr<FJsonObject>& Params)
{
    ...
    UWidgetBlueprint* WBP = LoadWBPFromPath(Path);
    if (!WBP) return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("WBP not found"));
    UTextBlock* TB = Cast<UTextBlock>(FindWidgetByName(WBP, WidgetName));
    if (!TB) return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Widget '%s' not found or not a TextBlock"), *WidgetName));
...
TSharedPtr<FJsonObject> HandleSetProgressBarPercent(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (!PB) return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Widget '%s' not found or not a ProgressBar"), *WidgetName));
...
TSharedPtr<FJsonObject> HandleSetProgressBarFillColor(const TSharedPtr<FJsonObject>& Params)
{
    FString Path, WidgetName;
    if (!Params->TryGetStringField(TEXT("path"), Path) ||
        !Params->TryGetStringField(TEXT("widget_name"), WidgetName))
...
TSharedPtr<FJsonObject> HandleSetHorizontalBoxSlotFill(const TSharedPtr<FJsonObject>& Params)
{
    FString Path, WidgetName;
    if (!Params->TryGetStringField(TEXT("path"), Path) ||
        !Params->TryGetStringField(TEXT("widget_name"), WidgetName))
    ...
    if (!W) return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Widget '%s' not found"), *WidgetName));
    UHorizontalBoxSlot* HSlot = Cast<UHorizontalBoxSlot>(W->Slot);
    if (!HSlot) return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Widget '%s' is not in a HorizontalBox"), *WidgetName));
...
TSharedPtr<FJsonObject> HandleSetCanvasSlotAnchor(const TSharedPtr<FJsonObject>& Params)
{
    FString Path, WidgetName;
    if (!Params->TryGetStringField(TEXT("path"), Path) ||
        !Params->TryGetStringField(TEXT("widget_name"), WidgetName))
    ...
    if (!CSlot) return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Widget '%s' is not in a CanvasPanel"), *WidgetName));
    ...
    { CSlot->SetSize(Size); CSlot->SetAutoSize(false); }
    FVector2D Alignment;
...
TSharedPtr<FJsonObject> HandleDeleteWidgetFromTree(const TSharedPtr<FJsonObject>& Params)
{
    FString Path, WidgetName;
    if (!Params->TryGetStringField(TEXT("path"), Path) ||
        !Params->TryGetStringField(TEXT("widget_name"), WidgetName))
    ...
    if (!bRemoved)
        return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("RemoveWidget failed for '%s'"), *WidgetName));
    FinalizeWBP(WBP);
    TSharedPtr<FJsonObject> Ok = MakeShared<FJsonObject>();
    Ok->SetBoolField(TEXT("success"), true);
    return Ok;
...
REGISTER_MCP_COMMAND("create_umg_widget_blueprint",   &HandleCreateUMGWidgetBlueprint);
REGISTER_MCP_COMMAND("add_text_block_to_widget",      &HandleAddTextBlockToWidget);
REGISTER_MCP_COMMAND("add_widget_to_viewport",        &HandleAddWidgetToViewport);
REGISTER_MCP_COMMAND("add_button_to_widget",          &HandleAddButtonToWidget);
REGISTER_MCP_COMMAND("bind_widget_event",             &HandleBindWidgetEvent);
REGISTER_MCP_COMMAND("set_text_block_binding",        &HandleSetTextBlockBinding);
REGISTER_MCP_COMMAND("add_widget_to_tree",            &HandleAddWidgetToTree);
REGISTER_MCP_COMMAND("set_widget_text",               &HandleSetWidgetText);
REGISTER_MCP_COMMAND("set_progress_bar_percent",      &HandleSetProgressBarPercent);
REGISTER_MCP_COMMAND("set_progress_bar_fill_color",   &HandleSetProgressBarFillColor);
REGISTER_MCP_COMMAND("set_horizontal_box_slot_fill",  &HandleSetHorizontalBoxSlotFill);
REGISTER_MCP_COMMAND("set_canvas_slot_anchor",        &HandleSetCanvasSlotAnchor);
REGISTER_MCP_COMMAND("delete_widget_from_tree",       &HandleDeleteWidgetFromTree);
REGISTER_MCP_COMMAND("compile_widget_blueprint",      &HandleCompileWidgetBlueprint);
